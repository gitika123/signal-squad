...
type Accent = "violet" | "cyan" | "coral" | "amber";

interface Actor extends Circle {
  vx: number;
  vy: number;
  speed: number;
  cargo: number;
...
interface Signal extends Circle {
  phase: number;
}

interface Hazard extends Circle {
  vx: number;
  vy: number;
  speed: number;
  spin: number;
...
const WORLD = { width: 1000, height: 620 } as const;
const COLORS = {
  violet: "#9b8cff",
  cyan: "#6ce5d7",
  coral: "#ff6f91",
...
const ui = {
  time: element<HTMLElement>("timeValue"),
  score: element<HTMLElement>("scoreValue"),
  streak: element<HTMLElement>("streakValue"),
  shield: element<HTMLElement>("shieldValue"),
  shieldMeter: element<HTMLElement>("shieldMeter"),
  playerCargo: element<HTMLElement>("playerCargo"),
  botCargo: element<HTMLElement>("botCargo"),
  partyCode: element<HTMLElement>("partyCode"),
  feed: element<HTMLOListElement>("activityFeed"),
  startOverlay: element<HTMLElement>("startOverlay"),
  endOverlay: element<HTMLElement>("endOverlay"),
  endTitle: element<HTMLElement>("endTitle"),
  endCopy: element<HTMLElement>("endCopy"),
  bestScore: element<HTMLElement>("bestScore"),
  soundButton: element<HTMLButtonElement>("soundButton"),
  pauseButton: element<HTMLButtonElement>("pauseButton"),
};
...
const beacon: Circle = { x: WORLD.width / 2, y: WORLD.height / 2, radius: 48 };

const randomPoint = (margin = 48): Vector => ({
  x: margin + Math.random() * (WORLD.width - margin * 2),
  y: margin + Math.random() * (WORLD.height - margin * 2),
});

const safeRandomPoint = (): Vector => {
  let point = randomPoint(58);
  for (let attempt = 0; attempt < 8 && distance(point, beacon) < 125; attempt += 1) point = randomPoint(58);
  return point;
};

const addFeed = (tag: string, message: string, accent: Accent = "violet"): void => {
...
const playTone = (frequency: number, duration = 0.08, volume = 0.025): void => {
...
const burst = (position: Vector, color: string, count = 12): void => {
...
const spawnHazard = (): void => {
  ...
  if (edge === 0) point.y = 24;
  if (edge === 1) point.x = WORLD.width - 24;
  if (edge === 2) point.y = WORLD.height - 24;
  if (edge === 3) point.x = 24;
  hazards.push({
    ...point,
...
const startGame = (): void => {
  ...
  pulses = [];
  resetActors();
  for (let index = 0; index < 8; index += 1) spawnSignal();
...
const finishGame = (connectionLost: boolean): void => {
  phase = "ended";
  keys.clear();
...
const collectSignal = (actor: Actor, name: string): void => {
  actor.cargo += 1;
  score += 20;
...
const deliverCargo = (actor: Actor, name: string): void => {
  if (actor.cargo === 0 || distance(actor, beacon) > beacon.radius + 22) return;
...
const usePulse = (): void => {
  if (phase !== "playing" || pulseCooldown > 0) return;
  pulseCooldown = 1.6;
...
const nearestSignalTo = (actor: Actor): Signal | undefined => {
...
const updateNova = (delta: number): void => {
...
const updateSignals = (): void => {
  ...
  while (signals.length < 8) spawnSignal();
  deliverCargo(player, "You");
  deliverCargo(nova, "Nova");
};

const updateHazards = (delta: number): void => {
...
const updateEffects = (delta: number): void => {
  pulses = pulses
...
const update = (delta: number): void => {
  ...
  updateHazards(delta);
  updateUi();
  if (shield <= 0) finishGame(true);
  else if (timeLeft <= 0) finishGame(false);
};
...
const drawSignal = (signal: Signal): void => {
...
const drawActor = (actor: Actor, label: string): void => {
...
const drawHazard = (hazard: Hazard): void => {
  context.save();
...
window.addEventListener("keydown", (event) => {
  ...
  if (event.code === "Space" && !event.repeat) usePulse();
...
element<HTMLButtonElement>("startButton").addEventListener("click", startGame);
element<HTMLButtonElement>("restartButton").addEventListener("click", startGame);
...
element<HTMLButtonElement>("copyButton").addEventListener("click", async (event) => {
